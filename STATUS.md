...
- **Google Sheets read widget** — an optional home widget (`src/components/shared/sheet-widget.tsx` + `src/lib/sheets.ts`) reads a range from a Google Sheet via the official Sheets API v4 (read-only, plain API key, no OAuth — the sheet must be shared "anyone with link can view"). Server-side fetch + 5min cache. The first row is treated as a header; up to 6 rows render with a "+N more" note, and the card links out to the sheet. Configured with `GOOGLE_SHEETS_API_KEY` + `GOOGLE_SHEET_ID` (+ optional `GOOGLE_SHEET_RANGE`, `GOOGLE_SHEET_LABEL`). The widget only appears on the home grid when configured, so the main screen stays calm for setups that don't use it.
